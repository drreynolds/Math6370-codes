...
int main(int argc, char* argv[]) {
  ...
  // local variables
  int m, n, i, j, is, ie, ierr, numprocs, myid;
  ...
  is = ((int) (1.0*m/numprocs))*myid;
  ie = ((int) (1.0*m/numprocs))*(myid+1);
  if (myid == numprocs-1)  ie = m;

  // allocate the matrix and vectors
  // (break up matrix, but store entire vectors on every proc)
  A = new double*[ie-is];
  for (i=0; i<ie-is; i++)  A[i] = new double[n];
  x = new double[n];
  b = new double[m];
  myb = new double[m];
  ...
  for (i=is; i<ie; i++) 
    for (j=0; j<n; j++) 
      A[i-is][j] = 1.0/(1.0 + (i-j)*(i-j));
  for (i=0; i<m; i++)  b[i] = 1.0;
  ...
  if (myid == 0) {
    ...
    fptr.open("b_mpi2.txt", std::fstream::out);
    for (i=0; i<m; i++)  fptr << b[i] << "\n";
    fptr.close();
  }

  // free matrix and vectors 
  for (i=0; i<ie-is; i++)  delete[] A[i];
  delete[] A;
  delete[] x;
  delete[] b;
  delete[] myb;
