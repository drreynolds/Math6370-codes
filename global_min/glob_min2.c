...
int main(int argc, char* argv[]) {
  ...
  searchpts[0] = malloc(npts * sizeof(double));
  searchpts[1] = malloc(npts * sizeof(double));
  searchvals = malloc(npts * sizeof(double));
  for (i=1; i<=nx*ny; i++) {
...
double maxval(double *v, int n) {
  double result = v[0];
  int i;
  for (i=0; i<n; i++)  result = (result > v[i]) ? result : v[i];
  return result;
}
