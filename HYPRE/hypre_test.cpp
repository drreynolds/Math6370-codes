...
int main(int argc, char *argv[]) {
  ...
  int Nx, Ny, Px, Py, maxit, relch, rlxtype, npre, npost;
  double mu_x, mu_y, delta, xL, xR, yL, yR;
  if (myid == 0) {
    FILE* FID = fopen("input_params.txt","r");
  ...
  int ibuff[] = {Nx, Ny, Px, Py, maxit, relch, rlxtype, npre, npost};
  double dbuff[] = {mu_x, mu_y, delta, xL, xR, yL, yR};
  ierr = MPI_Bcast(ibuff, 9, MPI_INT, 0, MPI_COMM_WORLD);
  if (ierr != MPI_SUCCESS) {
  ...
  HYPRE_StructPFMGSetMaxIter(solver, maxit);
  HYPRE_StructPFMGSetRelChange(solver, relch);
  HYPRE_StructPFMGSetRelaxType(solver, rlxtype);
  HYPRE_StructPFMGSetNumPreRelax(solver, npre);
  HYPRE_StructPFMGSetNumPostRelax(solver, npost);
  HYPRE_StructPFMGSetTol(solver, delta);
  HYPRE_StructPFMGSetLogging(solver, 1);
  HYPRE_StructPFMGSetup(solver, A, bvec, xvec);
  ...
  HYPRE_StructPFMGSolve(solver, A, bvec, xvec);
  ...
  double finalresid;
  HYPRE_StructPFMGGetFinalRelativeResidualNorm(solver, &finalresid);
  HYPRE_Int its;
  HYPRE_StructPFMGGetNumIterations(solver, &its);
  if (outproc)
    printf("   lin resid = %.1e (tol = %.1e), its = %i\n",
	   finalresid, delta, (int) its);
